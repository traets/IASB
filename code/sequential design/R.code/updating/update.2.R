...
#set
n_alts<-2
beta_true<-c(0.4, 0.2, 0.3, -0.4, 0.8, 0.1, -0.8, 1.2)
beta_prior<-c(0.1, 0.3, 0.1, -0.5, 0.1, 0.1, -0.2, 0.3)
var_prior<-c(4,4,4,4,4,4,4,4)
...
g_draws<-grid.MVT(beta_est, covar_post, grid)
...
cte<-sum(weights)
posterior<-weights/cte 

post_dens<-data.frame(cbind(g_draws, posterior))

sort(post_dens[posterior])
### (4) 
  post_dens[order(posterior),]
 
beta_post<-c(-2.19755596,  5.498421,  8.0964573,  5.679682e+00, 19.91018,  2.977612103,  3.412774386, 17.060036)

logLik(beta = beta_post,design, n_alts, Y )
