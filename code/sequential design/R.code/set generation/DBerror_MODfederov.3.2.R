...
Bayesian.set.info<- function (set) {
  ...
  for (r in 1:nrow(beta_samples)){ 
    ...
    Mset<-as.matrix(set)
    set_info<-(t(as.matrix(set)) %*% (diag(as.vector(exp(Mset%*%beta_r)/sum(exp(Mset %*% beta_r))))
             -(as.vector(exp(Mset%*%beta_r)/sum(exp(Mset %*% beta_r)))%*% t(as.vector(exp(Mset%*%beta_r)/
              sum(exp(Mset %*% beta_r)))))) %*% (as.matrix(set)))
...
Bayesian.set.info2 <- cmpfun(Bayesian.set.info)
...
Bayesian.design.info<- function (design){ 
  ...
  for (c_set in 1:n_sets){
    ...
    B_design_info[[c_set]]<-Bayesian.set.info2(set=choice_set)
