...
full_design<-full.profiles(levels_attributes=c(3,2,4),contrasts= "contr.sum")

### 2. Calculate information and D-efficiency #### 

#Specify:
n_alts=2                                 # alternatives per set
betas=c(1,1.5,-1.5,1,0,1)                      # Beta values 
...
information<-Fish.inf(set = 2)

# D-error for all choice sets
  #create vector to store
d.error<-as.vector((nrow(set.design)))
  
for (set in 1: nrow(set.design)){
     
    info<-Fish.inf(set)
    d.error[set]<-det(info)
  }

d.error

#plot
diff.prob<-probs.per.set[1]-probs.per.set[2]
x<-diff.prob$V1

plot(x,d.error,ylim = c(min(d.error),max(d.error)))
