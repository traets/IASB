...
## 2. calculate D-error for each possible set                         #
## 3. Select nr of initial sets based on d.errors                     #
## next: calculate Db-error / implement modified federov algorithm    #
#######################################################################
...
probs<-function (){
  ...
  if (length(betas)!=length(full_design)){stop ("nr of betas not correct")}
  ...
  total.U.set<-numeric(nrow(all.choice.sets))
  
  for (set in 1: nrow(all.choice.sets)){
    for (alt in 1: n_alts){
      prob.alt<-exp(utilities[all.choice.sets[set,alt]])
      total.U.set[set]<-total.U.set[set]+prob.alt
    }
  }
  
  #probabilities per alternative per set
  probs.per.set<-as.data.frame(matrix(nrow = nrow(all.choice.sets),ncol=n_alts))
  
  for (set in 1: nrow(all.choice.sets)){
    for (alt in 1: n_alts){
      probs.per.set[set,alt]<-exp(utilities[all.choice.sets[set,alt]])/total.U.set[set]
    }
  }
...
Fish.inf<- function (set){
  ...
  for(alt in 1:n_alts){ X[alt,]<-as.numeric(full_design[all.choice.sets[set,alt],])}
...
d_error<-function(){
  ...
  D<-as.vector(nrow(all.choice.sets))
...
betas<-c(0.5, 1.2, 0.2, 1.8, 1.3, 0.1)              # Beta values             
beta.var<-c(0.1,0.1,.1,0.1,0.1,0.1)                   # Beta variance 
COV<-diag(beta.var)
#---------------------------------------------------------------

#Functions
full_design<-full.profiles(contrasts= "contr.sum")
all.choice.sets<-full.choice.sets()
probs.per.set<-probs()
d.error<-d_error()

#sort
sorted.sets<-cbind(all.choice.sets,d.error)
sorted.sets<-sorted.sets[with(sorted.sets, order(d.error)), ]

#plot d.error and diff in prob. for each possible choice set 
