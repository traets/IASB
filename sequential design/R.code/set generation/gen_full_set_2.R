...
rand.profiles<-function (levels_attributes,contrast) {
  ...
  #contr.fun<-function(x){paste(x=, C)}
  #contrast.list<-lapply(X = list.attris, FUN = contr.fun)

  con<-list(Var1= "contr.sum", Var2= "contr.sum",Var3= "contr.sum",Var4= "contr.sum" )


  full_design_p<-as.data.frame(model.matrix(~ ., full_design, con))

  return(full_design)

}

full_design<-rand.profiles(levels_attributes=c(2,4,5,3),contrast=contr.sum)
