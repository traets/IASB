...
sum.info.temp<- function () {
  
  for ( c_set in 1:n_sets){
    
    #delete set and alt col   
    choice_set<-temp.design[temp.design$set==c_set,!(names(temp.design) %in% drops)]
...
sum.info.end<- function () {
  
  for ( c_set in 1:n_sets){
    
  
    choice_set<-end.design[end.design$set==c_set,!(names(end.design) %in% drops)]
...
#3.calculate error startdesign
INFO<-sum.information()
BASE<-det(INFO)^(-1/length(betas))

#create temporele design
temp.design<-start[,]

for (temp.row in 1:nrow(temp.design)){

  for (cand in 1:nrow(candidate_profiles)){
     
   
     #Change alternative
     temp.design[temp.row,1:length(betas)]<-candidate_profiles[cand,]
     
     #calculate error
     INFO<-sum.info.temp()
     NEW<-det(INFO)^(-1/length(betas))
     
     #if better --> keep
     if (is.nan(NEW)||is.infinite(NEW))
     {}
     else if (NEW <= BASE)
     {
       end.design<- as.data.frame(as.matrix(temp.design)) 
       BASE<<-NEW
     }
...
end.design<-as.data.frame(end.design)
