...
D.error<- function (design) {
  ...
  for (c_set in 1:n_sets)
  {
    ...
    set_info<-(t(as.matrix(choice_set)) %*% (diag(as.vector(exp(Mset%*%beta_samples)/sum(exp(Mset %*% beta_samples))))
             -(as.vector(exp(Mset%*%beta_samples)/sum(exp(Mset %*% beta_samples)))%*% t(as.vector(exp(Mset%*%beta_samples)/
              sum(exp(Mset %*% beta_samples)))))) %*% (as.matrix(choice_set)))
  ...
  D<-D_error<-(det(sum_M))^(-1/ncol(beta_samples))
  return(D)
}
DB.error<-function (){mean(apply(beta_samples,1,D.error))}

test<-D.error(design=start_des)

fri<-DB.error(design=start_des)
