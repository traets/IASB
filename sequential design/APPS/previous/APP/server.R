...
shinyServer(function(input, output) {
  ...
  # This renderUI function holds the primary actions of the survey area.
...
##
  output$choice.set <- renderDataTable({
      ...
      y<-map_resp(resp = responses, resp_options = answer_options, n_alts = n_alts)

      print(y)

      #update parameters
      draws<-imp_sampling(prior_mode = p_mod, prior_covar = p_cov, design = des, n_alts = n_alts, Y=y)
      w<-draws[[2]]
      sam<-draws[[1]]
  ...
  output$Question <- renderText({
    ...
    if (input$Click.Counter>0 & input$Click.Counter<=total_sets)
    {paste(h5("Question: ", input$Click.Counter))}

  })

 #save responses in vector
 observeEvent(input$Click.Counter, {responses[input$Click.Counter-1]<<-input$survey})



 output$Y <- renderText({paste(responses)})
